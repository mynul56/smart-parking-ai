...
export default function DashboardPage() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          // Default to Dhaka, Bangladesh if location is not available
          ...
          if (lot._id === data.lotId) {
            return {
              ...
              availableSlots: Math.max(
                0,
                Math.min(lot.totalSlots, lot.availableSlots + data.delta),
              ),
            };
          }
          return lot;
  ...
  // Calculate stats
  const totalSlots =
    lots?.reduce((sum: number, lot: any) => sum + lot.totalSlots, 0) || 0;
  const availableSlots =
    lots?.reduce((sum: number, lot: any) => sum + lot.availableSlots, 0) || 0;
  const occupancyRate =
    totalSlots > 0
      ? (((totalSlots - availableSlots) / totalSlots) * 100).toFixed(1)
      : 0;
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-2 text-gray-600">
          {user?.role === "admin" &&
            "Manage parking lots and monitor system performance"}
          {user?.role === "staff" &&
            "Monitor parking lots and assist customers"}
